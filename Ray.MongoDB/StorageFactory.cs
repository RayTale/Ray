    ...
    public class StorageFactory : IStorageFactory
    {
             ...
             where State : class, new()
        {
            if (config.Singleton)
            {
                ...
                {
                    return new ArchiveStorage<PrimaryKey, State>(serializer, config as StorageOptions);
                });
                return new ValueTask<IArchiveStorage<PrimaryKey, State>>(storage as IArchiveStorage<PrimaryKey, State>);
            }
            else
            {
                return new ValueTask<IArchiveStorage<PrimaryKey, State>>(new ArchiveStorage<PrimaryKey, State>(serializer, config as StorageOptions));
            }
        }
