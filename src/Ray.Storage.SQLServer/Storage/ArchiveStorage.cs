...
namespace Ray.Storage.SQLServer
{
          ...
          where StateType : class, new()
    {
        ...
        public ArchiveStorage(IServiceProvider serviceProvider, ISerializer serializer, StorageOptions config)
        {
            ...
            this.getLatestByStateIdSql = $"select top(1) Id,StartVersion,EndVersion,StartTimestamp,EndTimestamp,[Index],EventIsCleared FROM {tableName} where stateid=@StateId order by [index] desc";
            this.insertSql = $"INSERT into {tableName}(Id,stateid,StartVersion,EndVersion,StartTimestamp,EndTimestamp,[Index],EventIsCleared,data,IsOver,Version)VALUES(@Id,@StateId,@StartVersion,@EndVersion,@StartTimestamp,@EndTimestamp,@Index,@EventIsCleared,@Data,@IsOver,@Version)";
            this.updateOverSql = $"update {tableName} set IsOver=@IsOver where stateid=@StateId";
            this.updateEventIsClearSql = $"update {tableName} set EventIsCleared=1 where id=@Id";
        }
